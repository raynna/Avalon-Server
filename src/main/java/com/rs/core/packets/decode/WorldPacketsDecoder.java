...
public final class WorldPacketsDecoder extends Decoder {
	...
	private void handleEnterInteger(Player player, InputStream stream) {
		...
		if (player.getInterfaceManager().containsInterface(934)
			...
			if (index > 100) { index -= 100; dungeoneering = true; }
			player.closeInterfaces();
		...
		if (player.temporaryAttribute().get("serverupdate") == Boolean.TRUE) {
			...
			if (value > 30 || value <= 0) { player.message("Max is 30 minutes."); return; }
		...
		if (player.temporaryAttribute().get("doubleexp") == Boolean.TRUE) {
            ...
            if (value == 0) return;
			if (value > 5) { player.message("Max is 5. You can't go above that."); return; }
		...
		if (player.temporaryAttribute().get("bankAmount") != null) {
			...
			if (!player.getBank().hasBankSpace()) { player.message("Not enough bank space."); return; }
			if (bankedItem != null && bankedItem.getDefinitions().isNoted()) { player.message("You can't bank this item."); return; }
			if (bankedItem != null) {
	...
	private void handleEnterLongText(Player player, InputStream stream) {
		...
		if (player.temporaryAttribute().remove("entering_note") == Boolean.TRUE) {
			...
			player.getPackets().sendGameMessage("You have changed your password, your new password is '" + value + "'.");
			return;
			...
			if (other == null) { player.message("Target offline, or does not exist."); return; }
			...
			Player other = World.getPlayerByDisplayName(value);
			if (other == null) { player.message("Offline, or does not exist."); return; }
	...
	private void handleEnterName(Player player, InputStream stream) {
		...
		return;
			...
			Player other = World.getPlayerByDisplayName(value);
			if (other == null) { player.getPackets().sendGameMessage("Couldn't find player."); return; }
			ClanWars clan = other.getCurrentFriendChat() != null ? other.getCurrentFriendChat().getClanWars() : null;
			if (clan == null) { player.getPackets().sendGameMessage("This player's clan is not in war."); return; }
