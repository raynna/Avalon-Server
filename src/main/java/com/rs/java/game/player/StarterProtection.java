...
public class StarterProtection {
	...
	public static final void sendStarterPack(Player player) {
		...
		boolean active = player.getVarsManager().getBitValue(1485) == 1;
		player.getVarsManager().sendVarBit(1485, active ? 0 : 1, true);
		player.getVarsManager().forceSendVarBit(9816, player.getPrayer().getPrayerPoints());
		player.refreshHitPoints();
		player.getPrayer().refresh();
		player.toggles.put("ONEXPPERHIT", true);
		...
		player.toggles.put("DROPVALUE", 25000);
