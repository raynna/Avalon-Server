...
import com.rs.java.game.player.actions.combat.lunarspells.CureMe;
import com.rs.java.game.player.actions.combat.lunarspells.CureOther;
import com.rs.java.game.player.actions.combat.lunarspells.DisruptionShield;
import com.rs.java.game.player.actions.combat.lunarspells.EnergyTransfer;
import com.rs.java.game.player.actions.combat.lunarspells.HealGroup;
import com.rs.java.game.player.actions.combat.lunarspells.HealOther;
import com.rs.java.game.player.actions.combat.lunarspells.Humidify;
import com.rs.java.game.player.actions.combat.lunarspells.MagicImbue;
import com.rs.java.game.player.actions.combat.lunarspells.MonsterExamine;
import com.rs.java.game.player.actions.combat.lunarspells.NPCContact;
import com.rs.java.game.player.actions.combat.lunarspells.PlankMake;
import com.rs.java.game.player.actions.combat.lunarspells.StatRestoreShare;
import com.rs.java.game.player.actions.combat.lunarspells.StatSpy;
import com.rs.java.game.player.actions.combat.lunarspells.StringJewellery;
import com.rs.java.game.player.actions.combat.lunarspells.SuperGlassMake;
import com.rs.java.game.player.actions.combat.lunarspells.TuneBaneOre;
...
public class LunarMagicks {
            ...
            if (s != null) {
                ...
                if (s.getRune() != null) {
                    ...
                    for (Item originalRune : runesCopy) {
						...
						if (!hasRune(player, rune.getId(), rune.getAmount())) {
							canCast = false;
                        }
                        if (!canCast) {
                            player.getPackets().sendGameMessage(
	...
	private static boolean hasRune(Player player, int runeId, int amount) {
		if (hasRunePouch(player)) {
			for (Item item : player.getInventory().getItems().toArray()) {
				if (item == null || item.getId() != Item.getId("item.rune_pouch"))
					continue;

				ItemMetadata meta = item.getMetadata();
				if (meta instanceof RunePouchMetaData) {
					RunePouchMetaData pouch = (RunePouchMetaData) meta;
					int count = pouch.getRunes().getOrDefault(runeId, 0);
					if (count >= amount)
						return true;
				}
			}
		...
		return player.getInventory().getNumberOf(runeId) >= amount;
	}
