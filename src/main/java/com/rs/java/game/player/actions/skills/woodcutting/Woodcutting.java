package com.rs.java.game.player.actions.skills.woodcutting;
...
import com.rs.java.game.player.actions.skills.woodcutting.LumberjackOutfit.Pieces;
        ...
        if (doubleLoot(player, true)) {
            if (definitions != TreeDefinitions.VINES && definitions != TreeDefinitions.IVY) {
                if (player.getInventory().hasFreeSlots())
                    ...
                    GroundItems.updateGroundItem(new Item(definitions.getLogsId(), 1), player.getLocation(), player);
                totalXp *= 2;
            }
        }
