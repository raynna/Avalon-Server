...
public class CustomStore extends CustomStoreData {
    ...
    public void sendValue(Player player, int slotId) {
        ...
        if (!isGeneralStore(getShopId())) {
            player.message("You can't sell " + item.getDefinitions().getName() + " to this shop.");
            return;
        }
        int price = (int) (EconomyPrices.getPrice(originalId) * 0.33);
        if (price == -1) {
            player.message("You can't sell free items.");
            return;
        }
        player.message(item.getDefinitions().getName() + ": shop will buy for: "
        ...
        if (TYPE == SKILLCAPE) {
            for (Item item : SkillcapeStore.capes)
                if (itemId == item.getId()) {
                    ...
                    player.message(ItemDefinitions.getItemDefinitions(itemId).getName()
    ...
    public void sendBuy(int itemId, int amount) {
        ...
        if (canBuy) {
            ...
            if (amount * defaultPrice > (points != -1 ? points : player.getTotalCoins())) {
                ...
                price = defaultPrice * amount;
            if (amount < 1) {
                player.message("You don't have enough " + getPointsName(TYPE, SHOP) + " to buy this.");
                return;
            }
