		...
		if (stage == 0) {
			...
			if (componentId == OPTION_1) {
				if (player.getAppearance().getBootsColor() == 500) {
					...
					player.getAppearance().setBootsColor(player.getAppearance().getBootsColor() + 1);
					player.getAppearance().generateAppearenceData();
					sendOptionsDialogue("Choose an option", "Next Colour", "Previous Colour", "Enter a Colour Id",
							"Check Colour Id");
				}
			} else if (componentId == OPTION_2) {
				if (player.getAppearance().getBootsColor() == 0) {
				...
				sendNPCDialogue(570, 9827, "Your currently boot id is " + player.getAppearance().getBootsColor());
				stage = 0;
			}
		}
	}
