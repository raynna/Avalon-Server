...
class SeedTableEntry : DropEntry(-1, 1..1) {
    ...
    fun getRareEntries() =
        rareTable.mutableEntries().filterIsInstance<ItemWeightedEntry>()

    fun getUncommonEntries() =
        uncommonTable.mutableEntries().filterIsInstance<ItemWeightedEntry>()

    fun getTreeHerbEntries() =
        treeHerbTable.mutableEntries().filterIsInstance<ItemWeightedEntry>()
    ...
    fun roll(type: SeedTableType, player: Player, combatLevel: Int): Drop? {

        return when (type) {

            SeedTableType.GENERAL -> rollGeneral(player, combatLevel)
    ...
    private fun rollGeneral(player: Player, combatLevel: Int): Drop? {
        ...
        val sub = generalSubTables.firstOrNull {
    ...
    private fun weighted(size: Int, block: WeightedTable.() -> Unit): WeightedTable {
        val table = WeightedTable()
        table.setSize(size)
        table.block()
        return table
    ...
    private fun WeightedTable.add(item: String, amount: IntRange, weight: Int) {
