...
class SummoningCharms(
    ...
    fun getEntries(): List<CharmDisplayEntry> = entries.map { CharmDisplayEntry(it.itemId, it.amount, it.percent) }
    ...
    fun gold(
        amount: Int = 1,
        percent: Double,
    ) = add("item.gold_charm", amount, percent)

    fun green(
        amount: Int = 1,
        percent: Double,
    ) = add("item.green_charm", amount, percent)

    fun crimson(
        amount: Int = 1,
        percent: Double,
    ) = add("item.crimson_charm", amount, percent)

    fun blue(
        amount: Int = 1,
        percent: Double,
    ) = add("item.blue_charm", amount, percent)
        ...
        for (entry in entries) {
            ...
            if (roll < acc) {
                return Drop(
                    itemId = entry.itemId,
                    amount = entry.amount,
                    context = context.copy(dropSource = DropSource.CHARM),
                )
            }
        }
