...
object BarrowsChestTable {
    ...
    @JvmStatic
    fun barrowsUniqueChance(brothersKilled: Int): Int {
        return 450 - (58 * brothersKilled)
    }

    private val BROTHER_ITEMS = mapOf(
    ...
    @JvmField
    val BARROWS_CHEST_TABLE = dropTable(
        name = "Barrows Chest",
        category = MonsterCategory.MINIGAME,
        rolls = 1
    ) {
    ...
    @JvmStatic
    fun getAllBarrowsItems(): IntArray {
        return BROTHER_ITEMS.values
            .flatten()
            .map { Rscm.lookup(it) }
            .toIntArray()
    }
