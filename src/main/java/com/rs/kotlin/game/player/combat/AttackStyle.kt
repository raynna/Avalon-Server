...
enum class XpMode {
    ...
    companion object {
        ...
        @JvmStatic
        fun applyMultiplierFloor(
            xp: Int,
            mult: Fraction,
        ): Int = (xp * mult.num) / mult.den
    }
    ...
    fun distributeXp(
        ...
        when (this) {
            ATTACK -> {
                attacker.skills.addXp(Skills.ATTACK, baseXp.toDouble())
            }

            STRENGTH -> {
                attacker.skills.addXp(Skills.STRENGTH, baseXp.toDouble())
            }

            DEFENCE -> {
                attacker.skills.addXp(Skills.DEFENCE, baseXp.toDouble())
            }

            RANGED -> {
                attacker.skills.addXp(Skills.RANGE, baseXp.toDouble())
            }

            SHARED -> {
                if (type == CombatType.RANGED) {
                    val split = baseXp / 2
                    attacker.skills.addXp(Skills.RANGE, split.toDouble())
                    attacker.skills.addXp(Skills.DEFENCE, split.toDouble())
                } else {
                    val split = baseXp / 3
                    attacker.skills.addXp(Skills.ATTACK, split.toDouble())
                    attacker.skills.addXp(Skills.STRENGTH, split.toDouble())
                    attacker.skills.addXp(Skills.DEFENCE, split.toDouble())
                }
            }
        }
