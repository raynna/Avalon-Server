...
class HitBuilder(
    ...
    private var specialDamageMultiplier: Double = 1.0 // maps to your existing "specialMultiplier" in calculations
    ...
    private var spellId: Int = -1 // only relevant for magic
    ...
    /** Multiplies max hit (your "specialMultiplier") */
    fun damageMultiplier(mult: Double) = apply { specialDamageMultiplier = mult }

    /** Force damage roll range (still respects miss unless you use roll(GUARANTEED)) */
    fun damage(min: Int, max: Int) = apply {
        forcedMinDamage = min
        forcedMaxDamage = max
    }

    /** Force damage roll 0..amount */
    fun damage(amount: Int) = apply {
        forcedMinDamage = 0
    ...
    fun damageFromMaxHit(range: (maxHit: Int) -> IntRange) = apply {
        damageRangeFromMaxHit = range
    }

    fun bonus(block: (Hit) -> Unit) = apply {
        postRollBonus = block
    }

    fun maxHit(max: Int) = apply {
        maxHitOverride = max
    }

    fun baseDamage(amount: Int) = apply {
        baseDamage = amount
    }
    ...
    fun roll(mode: CombatHitRoll = CombatHitRoll.NORMAL): Hit {
        ...
        val look = forcedHitLook ?: when (type) {
        ...
        if (target is NPC && target.id == 4474) {
            hit.damage = if (damageRangeFromMaxHit != null) {
