...
object RscmGenerator {
    fun generateGroupedNpcRscm() {
        ...
        fun toBaseName(name: String): String = name.lowercase().replace("[^a-z0-9]+".toRegex(), "_").trim('_')
    ...
    fun generateGroupedObjectRscm() {
        val outputPath = Path("data/rscm/object_group.rscm")

        val grouped = mutableMapOf<Pair<String, String>, MutableList<Int>>()

        fun toBaseName(name: String): String = name.lowercase().replace("[^a-z0-9]+".toRegex(), "_").trim('_')

        fun normalizeOptions(options: Array<String>?): String {
            if (options == null) return "no_options"
        ...
        for (id in 0 until Utils.getObjectDefinitionsSize()) {
            val def = ObjectDefinitions.getObjectDefinitions(id)
            if (def.name.isNullOrBlank() || def.name == "null") continue

            val baseName = toBaseName(def.name)
            val optionKey = normalizeOptions(def.options)

            val key = baseName to optionKey
            grouped.getOrPut(key) { mutableListOf() }.add(def.id)
        }

        val entries = mutableListOf<String>()

        for ((key, ids) in grouped.toSortedMap(compareBy({ it.first }, { it.second }))) {
            val (base, optionSig) = key
            val groupName =
                if (optionSig == "no_options") {
                    base
                } else {
                    "${base}_$optionSig"
                }
        ...
        println("Generated grouped object mappings to $outputPath")
    }

    fun generateGroupedItemRscm() {
        ItemDefinitions.loadAll()
        val definitions = ItemDefinitions.getItemDefinitions()
        val outputPath = Path("data/rscm/item_group.rscm")

        val grouped = mutableMapOf<Pair<String, String>, MutableList<Int>>()

        fun toBaseName(name: String): String = name.lowercase().replace("[^a-z0-9]+".toRegex(), "_").trim('_')

        fun normalizeOption(option: String?): String {
            if (option.isNullOrBlank()) return "no_option"
            return option
                .lowercase()
                .replace("[^a-z0-9]+".toRegex(), "_")
                .trim('_')
        ...
        for (def in definitions) {
            if (def == null || !def.loaded || def.name.isNullOrBlank()) continue
            if (def.isNoted) continue // skip noted versions

            val baseName = toBaseName(def.name)
            val firstOption = def.inventoryOptions?.getOrNull(0)
            val optionKey = normalizeOption(firstOption)
        ...
        println("Generated grouped item mappings to $outputPath")
    }

    fun generateObjectRscm() {
        val outputPath = Path("data/rscm/object.rscm")
        ...
        for (id in 0 until Utils.getObjectDefinitionsSize()) {
            ...
            val base = toBaseName(def.name)
            val unique = uniqueName(base, def.id)
        ...
        println("Generated ${entries.size} object mappings to $outputPath")
    }
}
