...
ItemDefinitions {
    ...
    private static int getMatchScore(String a, String b) {
        if (a.equalsIgnoreCase(b)) return 0;
        if (b.startsWith(a)) return 1;
        if (b.contains(a)) return 2;
        return levenshteinDistance(a, b);
    }

    public static List<ItemDefinitions> searchItems(String query, int maxResults) {
        boolean noted = query.contains("#noted");
        String input = query.toLowerCase().replace("#noted", "").trim();

        List<ItemDefinitions> matched = new ArrayList<>();

        for (ItemDefinitions def : itemsDefinitions) {
            String name = def.name.toLowerCase();
            int score = getMatchScore(input, name);
            if (score <= 3) {
                matched.add(def);
            }
        }
        ...
        for (ItemDefinitions def : matched) {
            if (noted) {
                ...
                if (certId != -1 && certId != def.getId()) {
                    ItemDefinitions notedDef = ItemDefinitions.getItemDefinitions(certId);
                    if (notedDef != null) {
                        results.add(notedDef);
                        if (results.size() >= maxResults)
                            break;
                    }
                }
