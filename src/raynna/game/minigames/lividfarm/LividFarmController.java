package raynna.game.minigames.lividfarm;
...
import raynna.game.player.combat.magic.RuneService;
 ...
 * @author Zed & -Andreas
 */
public class LividFarmController extends Controller {
    ...
    public boolean processItemClick(int slotId, Item item, Player player) {
        if (item.getId() == 20702) {
            LividFarmController.makePlank(slotId, item, player);
            return true;
        }
        if (item.getId() == 20704) {
            LividFarmController.makeBunch(player);
            return true;
        }
        return false;
    }
