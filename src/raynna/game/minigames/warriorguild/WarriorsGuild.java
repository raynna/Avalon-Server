package raynna.game.minigames.warriorguild;
...
import raynna.game.npc.others.AnimatedArmor;
...
public class WarriorsGuild extends Controller {
			...
			if (object.getId() == 15647) {
				if (!inLobby) {
					if (player.getEquipment().getShieldId() == 8856) {
						ButtonHandler.sendTakeOff(player, Equipment.SLOT_SHIELD, -1);
	...
	private void balanceKeg(final WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (kegCount == 0)
					player.getAppearance().setRenderEmote(2671);
				kegCount++;
