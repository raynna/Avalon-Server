...
import raynna.game.player.cutscenes.NexCutScene;
...
public class NexCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (nex.getAttacksStage() == 0) {
			npc.animate(new Animation(6987));
			for (Entity t : npc.getPossibleTargets()) {
				Hit magicHit = npc.magicHit(t, defs.getMaxHit());
				ProjectileManager.send(Projectile.ELEMENTAL_SPELL, 471, npc, t, () -> {
					applyRegisteredHit(npc, t, magicHit);
					if (magicHit.getDamage() > 0 && Utils.roll(1, 6)) {
						t.getNewPoison().startPoison(30);
					}
				});
			}
			return npc.getAttackSpeed();
			...
			if (!nex.isTrapsSettedUp() && Utils.roll(1, 3)) {
				...
				for (Entity t : possibleTargets) {
					World.spawnObjectTemporary(new WorldObject(57261, 11, 0, t), 3);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (Utils.getDistance(npc, player) < 5) {
							...
							npc.animate(new Animation(6987));
							for (final Entity t : npc.getPossibleTargets()) {
								Hit magicHit = npc.magicHit(t, 290);
								ProjectileManager.send(Projectile.ELEMENTAL_SPELL, 374, npc, t, () -> {
									applyRegisteredHit(npc, t, magicHit);
									t.gfx(376);
									nex.heal(magicHit.getDamage() / 4);
			...
			ProjectileManager.send(Projectile.ELEMENTAL_SPELL, 374, npc, target, () -> {
			...
			for (final Entity t : npc.getPossibleTargets()) {
				Hit magicHit = npc.magicHit(t, defs.getMaxHit());
				ProjectileManager.send(Projectile.ELEMENTAL_SPELL, 362, npc, target, () -> {
					applyRegisteredHit(npc, t, magicHit);
					if (magicHit.getDamage() > 0 && Utils.roll(1, 6)) {
						t.addFreezeDelay(16, false);
						t.gfx(369);
					}
				});
			}
			...
			for (Entity t : npc.getPossibleTargets()) {
				Hit magicHit = npc.magicHit(t, defs.getMaxHit());
				ProjectileManager.send(Projectile.ELEMENTAL_SPELL, 471, npc, target, () -> {
					applyRegisteredHit(npc, t, magicHit);
				});
			}
			return npc.getAttackSpeed();
