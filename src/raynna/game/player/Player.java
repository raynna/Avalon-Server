...
import raynna.game.CreationKiln;
...
import raynna.game.item.itemdegrading.ChargesManager;
import raynna.game.map.MapBuilder;
import raynna.game.minigames.clanwars.FfaZone;
import raynna.game.minigames.clanwars.RequestController;
import raynna.game.minigames.clanwars.WarController;
import raynna.game.minigames.duel.DuelArena;
import raynna.game.minigames.duel.DuelRules;
import raynna.game.minigames.lividfarm.LividFarm;
import raynna.game.minigames.warriorguild.WarriorsGuild;
import raynna.game.npc.NPC;
import raynna.game.npc.familiar.Familiar;
import raynna.game.npc.pet.Pet;
...
import raynna.game.player.content.customtab.AchievementsTab;
import raynna.game.player.prayer.*;
import raynna.game.player.Ranks.Rank;
import raynna.util.*;
import raynna.game.player.Trade;
import raynna.game.player.grandexchange.GrandExchange;
import raynna.game.player.grandexchange.GrandExchangeManager;
import raynna.game.player.grandexchange.LimitedGEReader;
import raynna.game.player.queue.PlayerActionQueue;
import raynna.game.player.skills.mining.GoldenMiningOutfit;
import raynna.game.player.skills.woodcutting.LumberjackOutfit;
import raynna.game.player.tasksystem.TaskManager;
import raynna.data.rscm.Rscm;
import raynna.game.npc.KillCountContainer;
...
import raynna.game.player.actions.skills.hunter.HunterImplings;
import raynna.game.player.actions.skills.slayer.SlayerManager;
import raynna.game.player.actions.skills.summoning.Summoning;
import raynna.game.player.content.clans.ClanMember;
...
import raynna.game.player.content.pet.PetManager;
import raynna.game.player.content.pet.Pets;
import raynna.game.player.content.presets.PresetManager;
import raynna.game.player.content.quest.QuestList.Quests;
import raynna.game.player.content.quest.QuestManager;
...
import raynna.game.player.content.unlockables.UnlockableItems;
import raynna.game.player.controllers.Controller;
import raynna.game.player.controllers.CorpBeastController;
import raynna.game.player.controllers.CrucibleController;
import raynna.game.player.controllers.DTController;
...
import raynna.game.player.controllers.QueenBlackDragonController;
import raynna.game.player.controllers.WildernessController;
import raynna.game.player.controllers.ZGDController;
import raynna.game.player.controllers.castlewars.CastleWarsPlaying;
import raynna.game.player.controllers.castlewars.CastleWarsWaiting;
import raynna.game.player.controllers.fightpits.FightPitsArena;
import raynna.game.player.controllers.pestcontrol.PestControlGame;
import raynna.game.player.controllers.pestcontrol.PestControlLobby;
...
import raynna.core.packets.encode.WorldPacketsEncoder;
import raynna.util.HexColours.Colour;
import raynna.game.player.combat.CombatAction;
import raynna.game.player.combat.CombatStyle;
import raynna.game.player.combat.Weapon;
import raynna.game.player.combat.magic.MagicStyle;
import raynna.game.player.combat.melee.MeleeStyle;
import raynna.game.player.combat.range.RangedStyle;
import raynna.game.player.combat.special.CombatContext;
import raynna.game.player.combat.special.SpecialAttack;
import raynna.game.player.interfaces.HealthOverlay;
import raynna.game.player.interfaces.TimerOverlay;
import raynna.game.player.shop.ShopSystem;
import raynna.game.world.activity.pvpgame.PvPGame;
import raynna.game.world.activity.pvpgame.tournament.TournamentInstance;
import raynna.game.world.activity.pvpgame.tournament.TournamentRecovery;
...
import raynna.game.world.projectile.QueuedProjectile;
import raynna.game.world.pvp.PvpManager;
import raynna.game.world.pvp.SafeZoneService;
...
public class Player extends Entity {
    ...
    private transient raynna.game.player.queue.PlayerActionQueue actionQueue = new raynna.game.player.queue.PlayerActionQueue(this);

    public raynna.game.player.queue.PlayerActionQueue queue() {
        return actionQueue;
    }

    public void queue(Consumer<raynna.game.player.queue.PlayerActionQueue> block) {
    ...
    private transient raynna.game.player.conversation.DialogueManager newDialogueManager;
    private transient raynna.game.player.conversation.Dialogue newDialogue;
     ...
     * @Packets
     */
    Queue<QueuedWorldPacket> queuedWorldPackets = new ConcurrentLinkedQueue<>();
    ...
    public void init(Session session, String username, int displayMode, int screenWidth, int screenHeight, MachineInformation machineInformation, IsaacKeyPair isaacKeyPair) {
        ...
        newDialogueManager = new raynna.game.player.conversation.DialogueManager(this);
        ...
        queuedWorldPackets = new ConcurrentLinkedQueue<>();
    ...
    public void processQueuedWorldPackets() {
        int processed = 0;
        final int MAX_PACKETS_PER_TICK = 200;
        if (queuedWorldPackets.size() > 5000) {
            System.out.println("WARNING: Packet queue size = " + queuedWorldPackets.size());
        }
        while (processed < MAX_PACKETS_PER_TICK) {
            QueuedWorldPacket packet = queuedWorldPackets.poll();
            if (packet == null)
                break;

            WorldPacketsDecoder.decodeQueuedPacket(this, packet);
            processed++;
        }
    }
    ...
    public void run() {
        if (PlayerBotManager.isManagedBot(this)) {
            fastStartManagedBot();
            return;
        }
    ...
    private void fastStartManagedBot() {
        if (petManager == null) {
            petManager = new PetManager();
            petManager.setPlayer(this);
        }
    ...
    public raynna.game.player.conversation.Dialogue getNewDialogue() {
        return newDialogue;
    }

    public raynna.game.player.conversation.DialogueManager getNewDialogueManager() {
    ...
    public void queueWorldPacket(QueuedWorldPacket packet) {
        if (packet == null)
            return;
        queuedWorldPackets.add(packet);
    }
