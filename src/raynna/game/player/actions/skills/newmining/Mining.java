package raynna.game.player.actions.skills.newmining;
...
import raynna.game.player.actions.skills.newmining.GoldenMiningOutfit.Pieces;
import raynna.game.player.actions.skills.newmining.Ores.OreStore;
import raynna.game.player.actions.skills.newmining.Pickaxes.PickaxeStore;
		...
		for (PickaxeStore pickaxe : pickaxeData) {
			if (pickaxe != null) {
				if (player.hasTool(pickaxe.getPickAxeId())) {
					if (player.getSkills().getRealLevel(Skills.MINING) >= pickaxe.getLevelRequried())
						return pickaxe;
