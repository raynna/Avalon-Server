package raynna.game.player.content.dungeoneering;
...
import raynna.game.npc.dungeoneering.*;
import raynna.game.player.OwnedObjectManager;
import raynna.game.player.Player;
import raynna.game.player.Skills;
import raynna.game.player.actions.skills.mining.DungeoneeringMining;
import raynna.game.player.content.ItemConstants;
import raynna.game.player.content.dungeoneering.rooms.BossRoom;
import raynna.game.player.content.dungeoneering.rooms.HandledPuzzleRoom;
import raynna.game.player.content.dungeoneering.rooms.StartRoom;
import raynna.game.player.content.dungeoneering.rooms.puzzles.PoltergeistRoom;
import raynna.game.player.content.dungeoneering.skills.DungeoneeringFishing;
import raynna.game.player.controllers.DungeonController;
import raynna.util.Logger;
import raynna.util.Utils;
import raynna.game.npc.combatdata.*;
...
public class DungeonManager {
	...
	public void clearKeyList() {
		for (Player player : party.getTeam()) {
			player.getPackets().sendExecuteScript(6072);
	...
	public void setKey(DungeonConstants.KeyDoors key, boolean add) {
		...
		for (Player player : party.getTeam()) {
			...
			if (key.getIndex() != 64)
				player.getPackets().sendCSVarInteger(1875, keyList.contains(DungeonConstants.KeyDoors.GOLD_SHIELD) ? 1 : 0);
		}
	}
			...
			public void run() {
				...
				for (int i = 0; i < room.getRoom().getDoorDirections().length; i++) {
					...
					if (door.getType() == DungeonConstants.KEY_DOOR) {
						DungeonConstants.KeyDoors keyDoor = DungeonConstants.KeyDoors.values()[door.getId()];
		...
		if (party.getComplexity() >= 5 && Utils.random(3) == 0) { //sets thief chest
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				if (((x >= 6 && x <= 8) && b[2] != 0) || ((y >= 6 && y <= 8) && b[3] != 0))
					continue;
				if (!World.isFloorFree(0, toChunkX * 8 + x, toChunkY * 8 + y) || !World.isTileFree(0, toChunkX * 8 + x - Utils.DOOR_ROTATION_DIR_X[((rotation + 3) & 0x3)], toChunkY * 8 + y - Utils.DOOR_ROTATION_DIR_Y[((rotation + 3) & 0x3)], 0))
					continue;
	...
	public VisibleRoom getVisibleRoom(RoomReference reference) {
		if (reference.getX() >= visibleMap.length || reference.getY() >= visibleMap[0].length
				|| reference.getX() < 0 || reference.getY() < 0)
			// ArrayIndexOutOfBoundsException: -237
			return null;
	...
	public void spawnRandomNPCS(RoomReference reference) {
		...
		for (int i = 0; i < 1 + Utils.random(3); i++) { //all floors creatures, decreased
		...
		for (int i = 0; i < 1 + Utils.random(2); i++) { //this kind of floor creatures, decreased
		...
		//forgotten warrior
		if (Utils.random(3) == 0) //from 50% to 33
	...
	public DungeonNPC spawnNPC(int id, int rotation, WorldTile tile, RoomReference reference, int type, double multiplier) {
		...
		if (type == DungeonConstants.BOSS_NPC) {
			...
			/*else if (id >= 10770 && id <= 10775) //TODO rework this later.
				n = new FrostDragon(id, tile, this, multiplier);*/
		...
		n.setDirection(Utils.getAngle(Utils.DOOR_ROTATION_DIR_X[(rotation + 3) & 0x3], Utils.DOOR_ROTATION_DIR_Y[(rotation + 3) & 0x3]));
		return n;
	}
	...
	public CombatData getCombatData(boolean boss, int level) {
		...
		return new CombatData(
				...
				0, // strengthBonus
				0, // magicBonus
				0, // magicStrengthBonus
				0, // rangedBonus
				0, // rangedStrengthBonus
	...
	public void loadRewards() {
		...
		int uid = Utils.random(Integer.MAX_VALUE-20);
		...
		for (Player player : party.getTeam()) {
			...
			player.getPackets().sendExecuteScript(2275); //clears interface data from last run
			for (int i = 0; i < 5; i++) {
				...
				if (partyPlayer == null)
					continue;
				player.getPackets().sendCSVarString(310 + i, partyPlayer.getDisplayName());
			...
			player.getPackets().sendTextOnComponent(933, 331, Utils.formatTime((Utils.currentTimeMillis() - time) * 600));
			...
			if (!tickedOff) {
				skip: {if (true) {//TODO DungeonConstants.ENABLE_ALL_FLOORS
					for (int floor = 1; floor < party.getFloor(); floor++) {
						if (player.getDungManager().getMaxFloor() < floor)
							break skip;
						if (!player.getDungManager().isTickedOff(floor)) {
							player.getPackets().sendGameMessage("Since you have skipped a floor, floor " + floor + " was instead ticked-off.");
				...
				skip: {
					if (true) {//TODO DungeonConstants.ENABLE_ALL_FLOORS
						for (int floor = 1; floor < party.getFloor(); floor++) {
							if (player.getDungManager().getMaxFloor() < floor)
								break skip;
							if (!player.getDungManager().isTickedOff(floor)) {
			...
			int tokens = (int) totalXp; //(int) (totalXp / 10); x10 tokens cuz x10 xp
			...
			if (hsType != 2) {
				...
				//player.getDungManager().add(tokens);
			}
			player.getMusicsManager().forcePlayMusic(767); //fixed
			if (!tickedOff) {
			...
			//Highscores.updateDung(this, dungeon, (Utils.currentWorldCycle() - time) * 600L, timems, uid, teamSizeOrig, hsType, seed, player);
		}
		clearGuardians();
	}

	public static void main(String[] args) {
		System.out.println(getXPForFloor(3, 0));
	}

	public static int getXPForFloor(int floor, int type) {
		int points = 0;
		for (int i = 1; i <= Math.max(10, floor); i++)
	...
	public void load(long seed_) {
		party.lockParty();
		timems = Utils.currentTimeMillis();
		teamSizeOrig = party.getTeam().size();
		seed = seed_;
	...
	public void openMap(Player player) {
		...
		player.getPackets().sendExecuteScript(3277); //clear the map if theres any setted
		...
		for (int x = 0; x < visibleMap.length; x++) {
			for (int y = 0; y < visibleMap[x].length; y++) {
				if (visibleMap[x][y] != null) { //means exists
					...
					player.getPackets().sendExecuteScript(3278, protocol, getMapIconSprite(room, highLight), y, x);
					if (room.getRoom() instanceof StartRoom)
						player.getPackets().sendExecuteScript(3280, protocol, y, x);
					else if (room.getRoom() instanceof BossRoom)
						player.getPackets().sendExecuteScript(3281, protocol, y, x);
					if (room.hasNorthDoor() && visibleMap[x][y + 1] == null) {
						...
						player.getPackets().sendExecuteScript(3278, protocol, getMapIconSprite(DungeonConstants.SOUTH_DOOR, highLight), y + 1, x);
					}
					if (room.hasSouthDoor() && visibleMap[x][y - 1] == null) {
						Room unknownR = getRoom(new RoomReference(x, y - 1));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScript(3278, protocol, getMapIconSprite(DungeonConstants.NORTH_DOOR, highLight), y - 1, x);
					}
					if (room.hasEastDoor() && visibleMap[x + 1][y] == null) {
						Room unknownR = getRoom(new RoomReference(x + 1, y));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScript(3278, protocol, getMapIconSprite(DungeonConstants.WEST_DOOR, highLight), y, x + 1);
					}
					if (room.hasWestDoor() && visibleMap[x - 1][y] == null) {
						Room unknownR = getRoom(new RoomReference(x - 1, y));
						highLight = party.isGuideMode() && unknownR.isCritPath();
						player.getPackets().sendExecuteScript(3278, protocol, getMapIconSprite(DungeonConstants.EAST_DOOR, highLight), y, x - 1);
					}
				}
			}
		}
		...
		for (Player p2 : party.getTeam()) {
			...
			player.getPackets().sendExecuteScript(3279, p2.getDisplayName(), protocol, index++, reference.getY(), reference.getX());
		}
	}
