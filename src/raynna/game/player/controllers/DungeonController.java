package raynna.game.player.controllers;
...
import raynna.game.npc.dungeoneering.DivineSkinweaver;
import raynna.game.npc.dungeoneering.DungeonBoss;
import raynna.game.npc.dungeoneering.DungeonFishSpot;
import raynna.game.npc.dungeoneering.DungeonSlayerNPC;
import raynna.game.npc.dungeoneering.FrozenAdventurer;
import raynna.game.npc.dungeoneering.Gravecreeper;
import raynna.game.npc.dungeoneering.MastyxTrap;
import raynna.game.npc.dungeoneering.NightGazerKhighorahk;
import raynna.game.npc.dungeoneering.RuneboundBehemoth;
import raynna.game.npc.dungeoneering.Stomp;
import raynna.game.npc.dungeoneering.YkLagorThunderous;
import raynna.game.npc.familiar.Familiar;
import raynna.game.player.Inventory;
import raynna.game.player.Player;
import raynna.game.player.Skills;
import raynna.game.player.actions.HomeTeleport;
...
import raynna.game.player.actions.skills.mining.DungeoneeringMining.DungeoneeringRocks;
import raynna.game.player.actions.skills.mining.Mining;
import raynna.game.player.actions.skills.mining.MiningBase.PickAxeDefinitions;
import raynna.game.player.actions.skills.prayer.BoneOffering;
import raynna.game.player.actions.skills.prayer.Burying.Bone;
import raynna.game.player.actions.skills.smithing.DungeoneeringSmelting.SmeltingBar;
...
import raynna.game.player.content.Foods.Food;
import raynna.game.player.content.dungeoneering.Door;
import raynna.game.player.content.dungeoneering.DungeonConstants;
import raynna.game.player.content.dungeoneering.DungeonConstants.KeyDoors;
import raynna.game.player.content.dungeoneering.DungeonConstants.SkillDoors;
import raynna.game.player.content.dungeoneering.DungeonManager;
import raynna.game.player.content.dungeoneering.DungeonResourceShop;
import raynna.game.player.content.dungeoneering.DungeonUtils;
import raynna.game.player.content.dungeoneering.Room;
...
import raynna.game.player.content.dungeoneering.skills.DungeoneeringFarming;
import raynna.game.player.content.dungeoneering.skills.DungeoneeringFarming.Harvest;
import raynna.game.player.content.dungeoneering.skills.DungeoneeringFishing;
import raynna.game.player.content.dungeoneering.skills.DungeoneeringTraps;
import raynna.game.player.dialogues.Dialogue;
import raynna.game.player.dialogues.dungeoneering.DestroyCurrentStone;
...
public class DungeonController extends Controller {
	...
	public boolean sendDeath() {
		...
		if (player.getInventory().containsItem(DungeonConstants.GROUP_GATESTONE, 1)) {
			...
			GroundItems.addGroundItem(new Item(DungeonConstants.GROUP_GATESTONE), tile, player, false, 180, 2, -1);
		...
		if (player.getInventory().containsItem(DungeonConstants.GATESTONE, 1)) {
			...
			GroundItems.addGroundItem(new Item(DungeonConstants.GATESTONE), tile, player, false, 180, 2, -1);
	...
	public boolean canTakeItem(FloorItem item) {
		for (KeyDoors key : DungeonConstants.KeyDoors.values()) {
			if (item.getId() == key.getKeyId()) {
				...
				GroundItems.removeGroundItem(player, item, false);
				return false;
			}
		}
		if (item.getId() == DungeonConstants.GROUP_GATESTONE) {
			dungeon.setGroupGatestone(null);
			GroundItems.removeGroundItem(player, item, true);
			return true;
...
//Player player, WorldObject object, int interfaceId, Item item) {
	...
	public boolean canDropItem(Item item) {
		...
		if (item.getId() == DungeonConstants.GROUP_GATESTONE)
			...
			GroundItems.addGroundItem(item, currentTile, player, true, -1, 2, -1);
			player.getPackets().sendGameMessage("You place the gatestone. You can teleport back to it at any time.");
			return false;
		}
		GroundItems.addGroundItem(item, currentTile);
		return false;
	}
